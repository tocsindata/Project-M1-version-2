 ...
 * File: public_html/map-test6.php
 * Date: 2026-01-16
...
<head>
  ...
  <title>3D Terrain Test (Icons + Buildings + Legend Icons)</title>
...
<body>
  <div class="hud">
    ...
    <div>Terrain: <?= $maptiler_key !== '' ? 'ON' : 'OFF' ?> • Buildings: <?= $stadia_key !== '' ? 'ON' : 'OFF' ?></div>
    <?php if ($loadErrorEsc !== ''): ?>
  ...
  <script>
    ...
    // Legend colors + icon representative per type
    const typeColors = new Map();
    const typeIconUrl = new Map(); // event_type -> icon url (first seen)
    const palette = [
    ...
    function renderLegend() {
      ...
      holder.innerHTML = keys.length
        ? keys.map(k => {
            ...
            const icon = typeIconUrl.get(k) || '';

            const left = icon
              ? `<img class="icon" src="${escapeHtml(icon)}" alt="">`
              : `<span class="swatch" style="background:${c}"></span>`;

            return `<div class="row">${left}<span>${escapeHtml(k)}</span></div>`;
          }).join('')
    ...
    // If you provide a Stadia key, use their style JSON (typically OpenMapTiles schema incl. building layer).
    ...
    function safeIdFromUrl(url) {
      // Stable ID for any URL, regardless of extension/case.
    ...
    function normalizeGeojsonForIcons(geojson) {
      // Clone minimal structure and inject __icon_id for points with icon URLs.
      const out = JSON.parse(JSON.stringify(geojson || { type: 'FeatureCollection', features: [] }));
      ...
      for (const f of feats) {
        ...
        const iconUrl = (props.icon || '').toString().trim();

        if (iconUrl) {
          const id = safeIdFromUrl(iconUrl);
          f.properties = { ...props, __icon_id: id, __icon_url: iconUrl };
          iconUrls.add(iconUrl);
        } else {
          // Do not trust inbound helper fields
          const p2 = { ...props };
          delete p2.__icon_id;
          delete p2.__icon_url;
          f.properties = p2;
        }
      }
    ...
    function loadIconViaCanvas(iconUrl, maxSize = 32) {
      // Decodes PNG/JPG/GIF/etc via browser image decoder, then adds ImageData into the style.
      // GIF will render as first frame (no animation).
      const id = safeIdFromUrl(iconUrl);
      ...
      return new Promise((resolve) => {
        ...
        img.onload = () => {
          try {
            ...
            // Scale down to a sane marker size (keeps performance stable).
    ...
    async function loadIconsWithLimit(urls, limit = 6) {
      ...
      const workers = Array.from({ length: Math.max(1, limit) }, async () => {
        while (queue.length) {
          ...
          const id = await loadIconViaCanvas(url, 32);
    ...
    function addGeojsonLayers(geojsonOut, loadedIconIds) {
      const feats = Array.isArray(geojsonOut?.features) ? geojsonOut.features : [];

      // Legend model (colors + a representative icon per type)
      for (const f of feats) {
        const t = (f?.properties?.event_type || 'Unknown').toString();
        colorForType(t);

        const iconUrl = (f?.properties?.icon || '').toString().trim();
        if (iconUrl && !typeIconUrl.has(t)) {
          typeIconUrl.set(t, iconUrl);
        }
      }
      renderLegend();

      // Only keep __icon_id where we actually loaded the icon.
      // Otherwise circles remain visible.
      for (const f of feats) {
        const props = f?.properties || {};
        const iconId = (props.__icon_id || '').toString();

        if (iconId && !loadedIconIds.has(iconId)) {
          const p2 = { ...props };
          delete p2.__icon_id;
          delete p2.__icon_url;
          f.properties = p2;
        } else if (iconId) {
          // strip url helper to keep payload tidy in popups
          const p2 = { ...props };
          delete p2.__icon_url;
          f.properties = p2;
        }
      ...
      map.addSource('incidents', { type: 'geojson', data: geojsonOut });

      // Fallback circles (hidden only where loaded icons exist)
      map.addLayer({
      ...
      map.addLayer({
        ...
        type: 'symbol',
        source: 'incidents',
        filter: ['all', ['==', ['geometry-type'], 'Point'], ['has', '__icon_id']],
        layout: {
          'icon-image': ['get', '__icon_id'],
          'icon-size': 1,
    ...
    map.on('load', async () => {
      const { out: geojsonOut, iconUrls } = normalizeGeojsonForIcons(GEOJSON_IN);

      const urls = Array.from(iconUrls);
      const loadedIconIds = urls.length ? await loadIconsWithLimit(urls, 6) : new Set();

      addGeojsonLayers(geojsonOut, loadedIconIds);
