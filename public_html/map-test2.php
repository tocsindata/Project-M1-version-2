 ...
 * File: public_html/map-test2.php
 * Date: 2026-01-15
...
/*
  Usage:
    /map-test2.php
    /map-test2.php?geojson_url=https%3A%2F%2Fexample.com%2Fdata.geojson
*/
...
<head>
  ...
  <title>3D Terrain Test (Flat Map)</title>
...
<body>
  ...
  <script>
    ...
    async function ensureIconLoaded(iconUrl) {
    ...
    function enable3dBuildingsIfPossible() {
      ...
      // OpenMapTiles building layer is commonly named "building" with properties like height / render_height / etc.
      map.addLayer({
        id: '3d-buildings',
        source: sourceId,
    ...
    function addGeojsonLayers() {
      ...
      const firstIcon = feats.find(f => f?.properties?.icon)?.properties?.icon || '';
      if (firstIcon) {
        ensureIconLoaded(firstIcon).then((imgId) => {
          if (!imgId) return;
