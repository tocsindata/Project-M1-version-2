...
/*
  Usage:
    /map-test6.php
    /map-test6.php?geojson_url=https%3A%2F%2Fexample.com%2Fdata.geojson
*/
...
<head>
  ...
  <title>3D Terrain Test (Icons + Buildings)</title>
...
<body>
  ...
  <script>
    ...
    function safeIdFromUrl(url) {
      // Use a stable ID for any URL, regardless of extension/case.
      // (Assumes URL is ASCII-safe; your icon URLs are.)
    ...
    function addGeojsonLayers(geojsonOut, loadedIconIds) {
      ...
      // Critical: only keep __icon_id where we actually loaded the icon.
      ...
      // Fallback circles (always exist; hidden only where loaded icons exist)
      map.addLayer({
